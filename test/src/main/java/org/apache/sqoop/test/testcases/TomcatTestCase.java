...
abstract public class TomcatTestCase {
	 ...
	 *
	 * By default we will take a look for sqoop.integration.tmpdir property that
	 * is filled up by maven. If the test is not started from maven (IDE) we
	 * will pick up configured java.io.tmpdir value. The last results is /tmp/
	 * directory in case that no property is set.
	 */

	// NOTE: the default directory is /tmp/sqoop
	// where are the configs and logs will be picked up from and written to
	// respectively when we run an integration test
	// /tmp/sqoop/config
	// tmp/sqoop/log
	protected static final String TMP_PATH_BASE = System.getProperty(
			"sqoop.integration.tmpdir",
	 ...
	 *
	 * We will take TMP_PATH_BASE and append two subdirectories. First will be
	 * named after fully qualified class name of current test class, second
	 * directory will be named after current test method name. For example:
	 *
	...
	public static void startHadoop() throws Exception {
		...
		hadoopCluster = HadoopRunnerFactory.getHadoopCluster(
				System.getProperties(), HadoopLocalRunner.class);
		hadoopCluster.setTemporaryPath(TMP_PATH_BASE);
		hadoopCluster.setConfiguration(hadoopCluster
				.prepareConfiguration(new JobConf()));
		hadoopCluster.start();
	...
	public void startServer() throws Exception {
		...
		tmpPath = HdfsUtils.joinPathFragments(TMP_PATH_BASE, getClass()
				.getName(), name);
	...
	public TomcatSqoopMiniCluster createSqoopMiniCluster() throws Exception {
		return new TomcatSqoopMiniCluster(getSqoopMiniClusterTemporaryPath(),
	...
	/**
	 * Return execution engine test base directory.
	 *
	 * @return
	 */
	public String getHadoopTestDirectory() {
		return HdfsUtils.joinPathFragments(hadoopCluster.getTestDirectory(),
	 ...
	 *
	 * As the lines can be spread between multiple files the ordering do not
	 * make a difference.
	 *
	 * @param lines
	 * @throws IOException
	 */
	protected void assertHdfsTo(String... lines) throws IOException {
		HdfsAsserts.assertMapreduceOutput(hdfsClient, getHadoopTestDirectory(),
				lines);
	}
	 ...
	 */
	protected void assertHdfsToFiles(int expectedFiles) throws IOException {
		HdfsAsserts.assertMapreduceOutputFiles(hdfsClient,
				getHadoopTestDirectory(), expectedFiles);
	}
	 ...
	 *            Input file name
	 * @param lines
	 *            Individual lines that should be written into the file
	 * @throws IOException
	 */
	protected void createHdfsFromFile(String filename, String... lines)
			throws IOException {
		HdfsUtils.createFile(hdfsClient,
				HdfsUtils.joinPathFragments(getHadoopTestDirectory(), filename),
				lines);
	}
}
