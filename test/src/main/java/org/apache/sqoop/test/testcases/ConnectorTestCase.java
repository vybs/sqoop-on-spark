...
abstract public class ConnectorTestCase extends TomcatTestCase {
  ...
  protected void fillHdfsToConfig(MJob job, ToFormat output) {
    ...
    toConfig.getStringInput("toJobConfig.outputDirectory").setValue(getHadoopTestDirectory());
  }
  ...
  protected void fillHdfsFromConfig(MJob job) {
    MConfigList fromConfig = job.getJobConfig(Direction.FROM);
    fromConfig.getStringInput("fromJobConfig.inputDirectory").setValue(getHadoopTestDirectory());
  }
