...
public class MapreduceExecutionEngine extends ExecutionEngine {
  ...
  public void prepareJob(JobRequest jobRequest) {
    ...
    mrJobRequest.setMapOutputKeyClass(Text.class);
    mrJobRequest.setMapOutputValueClass(NullWritable.class);

    mrJobRequest.setOutputFormatClass(SqoopOutputFormat.class);
    mrJobRequest.setOutputKeyClass(Text.class);
    ...
    driverContext.setString(MRJobConstants.FROM_INTERMEDIATE_DATA_FORMAT,
        mrJobRequest.getIntermediateDataFormat(Direction.FROM).getName());
    driverContext.setString(MRJobConstants.TO_INTERMEDIATE_DATA_FORMAT,
        mrJobRequest.getIntermediateDataFormat(Direction.TO).getName());

    if(mrJobRequest.getExtractors() != null) {
      driverContext.setInteger(MRJobConstants.JOB_ETL_EXTRACTOR_NUM, mrJobRequest.getExtractors());
    }
  }
